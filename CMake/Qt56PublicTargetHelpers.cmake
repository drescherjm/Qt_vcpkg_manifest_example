...
function(__qt56_internal_strip_target_directory_scope_token target out_var)
...
function(__qt56_internal_static_link_order_public_test result)
    ...
    if(EXISTS "${QT_CMAKE_DIR}")
        ...
        set(test_source_basedir "${_qt56_cmake_dir}/${QT_CMAKE_EXPORT_NAMESPACE}")
    endif()
...
# Sets _qt56_link_order_matters flag for the target.
function(__qt56_internal_set_link_order_matters target link_order_matters)
    if(NOT TARGET ${target})
        message(FATAL_ERROR "Unable to set _qt56_link_order_matters flag. ${target} is not a target.")
    endif()
    ...
    set_target_properties(${target} PROPERTIES _qt56_link_order_matters "${link_order_matters}")
endfunction()

# Function combines __qt56_internal_static_link_order_public_test and
# __qt56_internal_set_link_order_matters calls on Qt::Platform target.
function(__qt56_internal_check_link_order_matters)
    __qt56_internal_static_link_order_public_test(
         link_order_matters
    )
    __qt56_internal_set_link_order_matters(
...
function(__qt56_internal_get_cmp0099_genex_check result)
    if(POLICY CMP0099)
...
function(__qt56_internal_check_cmp0099_available)
    ...
    set_target_properties(${platform_target} PROPERTIES
        _qt56_cmp0099_policy_check "${cmp0099_check}"
    )
...
function(__qt56_internal_process_dependency_object_libraries target)
    ...
    __qt56_internal_collect_dependency_object_libraries(${target} objects)
...
function(__qt56_internal_collect_dependency_object_libraries target out_var)
    ...
    __qt56_internal_collect_plugin_targets_from_dependencies(${target} plugin_targets)
    __qt56_internal_collect_dependency_plugin_object_libraries(${target}
        "${plugin_targets}"
        plugin_objects
    )
...
function(__qt56_internal_collect_dependency_plugin_object_libraries target plugin_targets out_var)
    ...
    foreach(plugin_target IN LISTS plugin_targets)
        __qt56_internal_collect_object_libraries_recursively(plugin_object_libraries
            "${QT_CMAKE_EXPORT_NAMESPACE}::${plugin_target}"
            ${target}
        )
        __qt56_internal_get_static_plugin_condition_genex("${plugin_target}" plugin_condition)
...
function(__qt56_internal_collect_object_libraries_recursively out_var target initial_target)
    get_property(processed_object_libraries GLOBAL PROPERTY _qt56_processed_object_libraries)
    ...
    foreach(lib IN LISTS libs interface_libs)
        ...
        if(TARGET ${lib})
            ...
            if(${lib} IN_LIST processed_object_libraries)
                ...
                set_property(GLOBAL APPEND PROPERTY _qt56_processed_object_libraries ${lib})
            endif()

            get_target_property(is_qt56_propagated_object_library ${lib}
                _is_qt56_propagated_object_library
            )
            if(is_qt56_propagated_object_library)
                list(APPEND object_libraries ${lib})
            else()
                __qt56_internal_collect_object_libraries_recursively(next_level_object_libraries
                    ${lib}
                    ${initial_target}
                )
...
function(_qt56_internal_should_not_promote_package_target_to_global target out_var)
    get_property(should_not_promote TARGET "${target}" PROPERTY _qt56_no_promote_global)
...
# first created.
#
# Uses __qt56_internal_walk_libs.
function(_qt56_internal_promote_3rd_party_link_targets_to_global target)
    __qt56_internal_walk_libs("${target}" _discarded_out_var _discarded_out_var_2
...
# etc).
function(_qt56_internal_is_internal_target target out_var)
    get_target_property(is_internal ${target} _qt56_is_internal_target)
    if(is_internal)
        set(value TRUE)
    else()
        set(value FALSE)
...
# Some targets like the Platform target is public, and can't have _qt56_is_internal_target set.
# But we still want to avoid promoting it to global. Setting this property achieves that.
function(_qt56_internal_should_skip_3rd_party_global_promotion target out_var)
    get_target_property(should_skip ${target} _qt56_should_skip_3rd_party_global_promotion)
    if(should_skip)
        set(value TRUE)
    else()
        set(value FALSE)
...
function(__qt56_internal_promote_target_to_global target)
...
# _qt56_internal_find_third_party_dependencies.
# Only does it when building Qt, but not when building user projects.
function(_qt56_internal_promote_3rd_party_provided_target_and_3rd_party_deps_to_global target)
    ...
    # which is set in .cmake.conf, because _qt56_internal_find_third_party_dependencies is called
    ...
    get_property(is_global TARGET "${target}" PROPERTY IMPORTED_GLOBAL)
    _qt56_internal_should_not_promote_package_target_to_global("${target}" should_not_promote)
    if(NOT is_global AND NOT should_not_promote)
        _qt56_internal_promote_3rd_party_target_to_global(${target})
        _qt56_internal_promote_3rd_party_link_targets_to_global("${target}")
    endif()
endfunction()
...
function(_qt56_internal_promote_3rd_party_target_to_global target)
    get_property(is_global TARGET ${target} PROPERTY IMPORTED_GLOBAL)

    if(NOT is_global)
        _qt56_internal_is_internal_target("${target}" is_internal)
        _qt56_internal_should_skip_3rd_party_global_promotion("${target}" should_skip)
        if(NOT is_internal AND NOT should_skip)
            __qt56_internal_promote_target_to_global("${target}")
        endif()
    endif()
endfunction()

function(__qt56_internal_promote_target_to_global_checked target)
...
function(__qt56_internal_promote_targets_in_dir_scope_to_global)
...
function(__qt56_internal_promote_targets_in_dir_scope_to_global_checked)
    if(QT_PROMOTE_TO_GLOBAL_TARGETS)
        __qt56_internal_promote_targets_in_dir_scope_to_global()
    endif()
endfunction()
...
# directory scope.
function(__qt56_internal_defer_promote_targets_in_dir_scope_to_global)
    get_directory_property(is_deferred _qt56_promote_targets_is_deferred)
    if(NOT is_deferred)
        set_property(DIRECTORY PROPERTY _qt56_promote_targets_is_deferred TRUE)

        if(CMAKE_VERSION VERSION_GREATER_EQUAL 3.19)
            cmake_language(DEFER CALL __qt56_internal_promote_targets_in_dir_scope_to_global_checked)
        endif()
    endif()
endfunction()

function(_qt56_internal_set_up_static_runtime_library target)
...
function(_qt56_internal_warn_about_example_add_subdirectory)
