...
function(_qt56_internal_re_escape out_var str)
    string(REGEX REPLACE "([][+.*()^])" "\\\\\\1" regex "${str}")
    set(${out_var} ${regex} PARENT_SCOPE)
endfunction()

function(_qt56_internal_set_rpath)
    if(NOT CMAKE_HOST_UNIX OR CMAKE_HOST_APPLE)
        message(WARNING "_qt56_internal_set_rpath is not implemented on this platform.")
        return()
    endif()
...
function(_qt56_internal_get_rpath_origin out_var)
...
# The hooks are run at the end of _qt56_internal_generic_deployqt.
#
# Every hook is passed the parameters of _qt56_internal_generic_deployqt plus the following:
# RESOLVED_DEPENDENCIES: list of resolved dependencies that were installed.
function(_qt56_internal_add_deployment_hook function_name)
    set_property(GLOBAL APPEND PROPERTY QT_INTERNAL_DEPLOYMENT_HOOKS "${function_name}")
endfunction()

# Run all registered deployment hooks.
function(_qt56_internal_run_deployment_hooks)
...
function(_qt56_internal_generic_deployqt)
    set(no_value_options
        NO_TRANSLATIONS
        VERBOSE
    )
    ...
    if("${arg_POST_EXCLUDE_REGEXES}" STREQUAL "")
        ...
        foreach(path IN LISTS QT_DEPLOY_IGNORED_LIB_DIRS)
            _qt56_internal_re_escape(path_rex "${path}")
    ...
    if(__QT_DEPLOY_MUST_ADJUST_PLUGINS_RPATH)
        _qt56_internal_get_rpath_origin(rpath_origin)
    endif()
    ...
    if(NOT arg_NO_TRANSLATIONS)
        qt56_deploy_translations()
    endif()

    _qt56_internal_run_deployment_hooks(${ARGV} RESOLVED_DEPENDENCIES ${resolved})
endfunction()

function(qt56_deploy_runtime_dependencies)
    ...
    list(APPEND tool_options ${__qt56_deploy_tool_extra_options})
    ...
    if(__QT_DEPLOY_TOOL STREQUAL "GRD")
        ...
        _qt56_internal_generic_deployqt(
            LIB_DIR "${arg_LIB_DIR}"
            PLUGINS_DIR "${arg_PLUGINS_DIR}"
            ${tool_options}
        )
...
function(qt56_deploy_translations)
